...
javascript: (function () {
	...
	let selectionRemade = selectionArray.map(item => (noCapitalization.includes(item) == true) ? 
						 item = item : item = item[0].toUpperCase() + item.slice(1)).join(' ');
...
javascript: (function () {
	...
	let selectionString = ' ' + window.getSelection().toString();
	let output = selectionString.substr(1, 1).toUpperCase() + selectionString.slice(2) + ', ' + 
	    currentSegmentText.substr(0, 1).toLowerCase() + currentSegmentText.slice(1).replace(selectionString, '');
	currentSegment.innerText = output;
})()
...
javascript: (function () {
	navigator.clipboard.readText().then(clipboardText => {
		...
		for (i = 0; i < clipboardArray.length; i++) {
			statement = 'document.activeElement.closest(&quot;tbody&quot;)' + '.nextElementSibling'.repeat(i) + 
				'.getElementsByClassName(&quot;l-content-editor__view l-content-editor__view_editor&quot;)[1].innerText=clipboardArray[i]';
			eval(statement);
		}
	})
})()
